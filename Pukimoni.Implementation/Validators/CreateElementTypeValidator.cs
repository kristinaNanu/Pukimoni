...
{
    public class CreateElementTypeValidator : AbstractValidator<LookupDto>
    {
        public CreateElementTypeValidator(PukimoniContext context)
        {
            RuleFor(x => x.Name)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage("Name is required")
                .Must(x => x.Length <= 15).WithMessage("Name can have 15 charachters")
                .Must(x => !context.ElementTypes.Any(y => y.Name == x))
